...
# MAGIC %run /Workspace/Users/kaushaldhanani10@gmail.com/CONSOLIDATED_PIPELINE/2_dimesion_data_processing/utilities
...
base_path = f's3://sportsbar-kd/{data_source}'
...
df = spark.read.options(header=True, inferSchema=True).csv(f"{landing_path}/*.csv").withColumn("read_timestamp", F.current_timestamp()).withColumn("order_qty", F.col("order_qty").cast("double")).select("*", "_metadata.file_name", "_metadata.file_size")
...
df = spark.read.options(header=True, inferSchema=True).csv(f"{landing_path}/*.csv")\
    ...
    .withColumn("order_qty", F.col("order_qty").cast("double"))\
